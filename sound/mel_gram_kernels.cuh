...
// FFT and Mel-gram computation function
template <typename T, typename U>
void compute_mel_gram(const T* d_input, T* d_output, int num_frames, int num_features, const U* d_mel_filters, int num_fft) {
    // Allocate FFT plan
    cufftHandle plan;
    cufftPlan1d(&plan, num_fft, CUFFT_R2C, num_frames);

    // Allocate memory for FFT results
    cufftComplex* d_fft_output;
    cudaMalloc(&d_fft_output, sizeof(cufftComplex) * num_frames * (num_fft / 2 + 1));

    // Perform FFT
    cufftExecR2C(plan, (cufftReal*)d_input, d_fft_output);

    // Compute power spectrum
    dim3 grid((num_frames * (num_fft / 2 + 1) + 255) / 256);
    dim3 block(256);
    compute_power_spectrum<<<grid, block>>>(d_fft_output, d_input, num_frames, num_fft / 2 + 1);

    // Apply mel filters and logarithmic compression
    apply_mel_filters_and_log<<<grid, block>>>(d_input, d_output, num_features, num_frames, d_mel_filters, num_fft / 2 + 1);
